...
export const load: ServerLoad = async({url})=>{
    ...
    let queryBase = db.selectFrom('family_background_question');
    if (searchTerm) {
    ...
    const antecedentesFamiliares = await queryBase
        .selectAll()
        .orderBy('id', 'asc')
        .limit(pageSize)
    ...
    delete: async ({ request }) => {
        ...
        try {
            ...
            const deleteResult = await db
                .deleteFrom('family_background_question')
                .where('family_background_question.id', '=', selectedItemId)
                .executeTakeFirst();

            // `numDeletedRows` es un BigInt, por eso se compara con 0n
            if (deleteResult.numDeletedRows === 0n) {
                return fail(404, { message: `Antecedente familiar con ID ${selectedItemId} no encontrado.` });
            }

            // Si todo sale bien, SvelteKit recargará los datos de `load` automáticamente.
            return { success: true, message: `Antecendente familiar ${selectedItemId} eliminado.` };
