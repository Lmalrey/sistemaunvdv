...
import { antecedentesFamiliaresSchema, editAntecedentesFamiliaresSchema } from '../../schema';
...
export const load: PageServerLoad = async ({ params }) => {
    ...
    const antecedenteFamiliar = await db
        .selectFrom('family_background_question')
        .selectAll()
        .where('id', '=', id)
        .executeTakeFirst();

    if (!antecedenteFamiliar) {
        throw error(404, 'Antecedente familiar no encontrado');
    }
    ...
    const dataForForm = {
        id: antecedenteFamiliar.id,
        pregunta: antecedenteFamiliar.question
    };
    // Inicializamos Superforms con el esquema y los datos cargados.
    const form = await superValidate(dataForForm, zod4(editAntecedentesFamiliaresSchema));
...
export const actions: Actions = {
    default: async ({ request, params }) => {
        ...
        try {
            await db
                .updateTable('family_background_question')
                .set({
        ...
        throw redirect(303, '/private/configuracion/antecedentes_familiares');
    }
};
