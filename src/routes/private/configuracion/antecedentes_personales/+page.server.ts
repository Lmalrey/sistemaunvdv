...
export const load: ServerLoad = async({url})=>{
    ...
    let queryBase = db.selectFrom('personal_background_question');
    if (searchTerm) {
        queryBase = queryBase.where('question', 'ilike', `%${searchTerm}%`);
    }
    ...
    const antecedentesPersonales = await queryBase
        .selectAll()
        .orderBy('id', 'asc')
        .limit(pageSize)
    ...
    delete: async ({ request }) => {
        ...
        try {
            ...
            const deleteResult = await db
                .deleteFrom('personal_background_question')
                .where('personal_background_question.id', '=', selectedItemId)
                .executeTakeFirst();

            // `numDeletedRows` es un BigInt, por eso se compara con 0n
            if (deleteResult.numDeletedRows === 0n) {
                return fail(404, { message: `Antecedente personal con ID ${selectedItemId} no encontrado.` });
            }

            // Si todo sale bien, SvelteKit recargará los datos de `load` automáticamente.
            return { success: true, message: `Antecendente personal ${selectedItemId} eliminado.` };
