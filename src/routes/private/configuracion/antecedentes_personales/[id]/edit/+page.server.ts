...
import { antecedentesPersonalesSchema, editAntecedentesPersonalesSchema } from '../../schema';
...
export const load: PageServerLoad = async ({ params }) => {
    ...
    const antecedentePersonal = await db
        .selectFrom('personal_background_question')
        .selectAll()
        .where('id', '=', id)
        .executeTakeFirst();

    if (!antecedentePersonal) {
        throw error(404, 'Antecedente personal no encontrado');
    }
    ...
    const dataForForm = {
        id: antecedentePersonal.id,
        pregunta: antecedentePersonal.question
    };
    // Inicializamos Superforms con el esquema y los datos cargados.
    const form = await superValidate(dataForForm, zod4(editAntecedentesPersonalesSchema));
...
export const actions: Actions = {
    default: async ({ request, params }) => {
        ...
        try {
            await db
                .updateTable('personal_background_question')
                .set({
        ...
        throw redirect(303, '/private/configuracion/antecedentes_personales');
    }
};
