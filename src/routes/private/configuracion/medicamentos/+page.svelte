<script lang="ts">
    ...
    // El tipo de dato que viene de `load` ahora incluye `brand_name`
    type MedicineWithBrand = {
        id: number;
        name: string;
        brand_name: string;
    };
    ...
    // --- Definición de Columnas para la tabla de Medicamentos ---
    const columns: ColumnDef<MedicineWithBrand>[] = [
        {
            accessorKey: 'id',
            header: 'ID',
        },
    ...
    const table = $derived(createSvelteTable({
        data: data.medicines, // Usamos 'medicines' en lugar de 'diagnosticos'
        columns,
    ...
    // El resto de la lógica (goToPage, handleSearchInput, modales) es idéntica
	...
	function handleEdit() {
		if(selectedItemId){
            // Actualizamos la ruta para editar medicamentos
            goto(`/private/configuracion/medicamentos/${selectedItemId}/edit`);
        }
		closeActionsModal();
	}
</script>

{#snippet ActionsCell(medicine: MedicineWithBrand)}
    <div class="actions-container">
        <button class="actions-button" onclick={()=>openActionsModal(medicine.id)}>
        ...
        {#if isModalOpen && selectedItemId === medicine.id}
    ...
    <div class="content-section">
        ...
        <div class="actions-bar">
            <div class="search-box">
                ...
                <input 
                    type="text" 
                    placeholder="Buscar medicamentos o marcas..."
                    bind:value={searchTerm}
                    oninput={handleSearchInput}
                >
            </div>
            <button class="add-button">
                <i class="fas fa-plus"></i> <a href="/private/configuracion/medicamentos/add">
                    Agregar medicamento
                </a>
            </button>
        </div>
        ...
        {:else}
            <p>
                {#if data.searchTerm}
                    No se encontraron medicamentos para "<strong>{data.searchTerm}</strong>".
                {:else}
