...
export const medicalRecordSchema = z.object({
  ...
  patient_id: z.coerce.bigint().refine((val) => val > 0n, {
    message: 'Debe seleccionar un paciente'
  }),
  doctor_id: z.coerce.bigint().refine((val) => val > 0n, {
    message: 'Debe seleccionar un doctor'
  }),
  tenant_id: z.coerce.bigint().refine((val) => val > 0n, {
  ...
  // El tipo numrange de postgres se maneja como un string. La validación regex es correcta para la entrada del usuario.
  ...
  diagnosis_ids: z.array(z.coerce.bigint()).min(1, 'Debe seleccionar al menos un diagnóstico')
});