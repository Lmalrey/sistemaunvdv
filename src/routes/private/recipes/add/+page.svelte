<script lang="ts">
	...
	// Variable derivada para filtrar los informes. Esto sigue siendo correcto y eficiente.
	...
	// Guardamos el ID del paciente anterior para detectar cuándo cambia.
	let previousPatientId = $state($form.patient_id);

	$effect(() => {
		// Este efecto se ejecuta cuando 'patient_id' o 'previousPatientId' cambian.
		if ($form.patient_id !== previousPatientId) {
			// Si el ID del paciente ha cambiado...
			$form.medical_record_id = ''; // ...reseteamos la selección del informe médico.
			previousPatientId = $form.patient_id; // ...y actualizamos el valor anterior para la próxima comparación.
		}
	});
	// --- FIN DE LA SOLUCIÓN ---
...
<div class="recipe-container">
	...
	<form method="POST" use:enhance>
		<!-- INICIO: SECCIÓN AÑADIDA PARA LA TABLA 'RECIPE' -->
			...
			<div class="form-grid">
				...
				<div class="full-width">
					...
					{#if $errors.medical_record_id}
						<span class="error">{$errors.medical_record_id}</span>
					{/if}
				</div>
			</div>
		</div>
				...
				<div class="medicine-entry">
					...
					<div class="form-grid">
						<div class="full-width">
							...
							<select
								name="medicines[{i}].medicine_id"
								id="medicine_id_{i}"
								bind:value={$form.medicines[i].medicine_id}
							>
							...
							{#if $errors.medicines?.[i]?.medicine_id}
								<span class="error">{$errors.medicines[i].medicine_id}</span>
							{/if}
						</div>
						...
						<div>
							...
							<input
								type="number"
								name="medicines[{i}].amount"
								id="amount_{i}"
								bind:value={$form.medicines[i].amount}
							/>
							{#if $errors.medicines?.[i]?.amount}
								<span class="error">{$errors.medicines[i].amount}</span>
							{/if}
						</div>
						...
						<div>
							...
							<select
								name="medicines[{i}].measurement_unit_id"
								id="unit_id_{i}"
								bind:value={$form.medicines[i].measurement_unit_id}
							>
							...
							{#if $errors.medicines?.[i]?.measurement_unit_id}
								<span class="error">{$errors.medicines[i].measurement_unit_id}</span>
							{/if}
						</div>
						...
						<div class="full-width">
							...
							<textarea
								name="medicines[{i}].indications"
								id="indications_{i}"
								bind:value={$form.medicines[i].indications}
							></textarea>
							{#if $errors.medicines?.[i]?.indications}
								<span class="error">{$errors.medicines[i].indications}</span>
							{/if}
						</div>
					</div>
					<button type="button" class="remove-btn" onclick={() => removeMedicine(i)}>
						Eliminar Medicamento
					</button>
				</div>
			{/each}
			{#if $errors.medicines?._errors}
				<span class="error full-width">{$errors.medicines._errors}</span>
			{/if}
